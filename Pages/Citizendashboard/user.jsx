      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex items-center gap-3 bg-white border border-slate-200 px-4 py-2.5 rounded-2xl shadow-sm">
            ...
            <span className="text-sm font-bold text-slate-700">{totalItems} Total Accounts</span>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-[24px] border border-slate-200 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
                    ...
                    <tr key={user._id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-6 py-5 whitespace-nowrap">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="text-sm font-bold text-slate-900 group-hover:text-blue-600 transition-colors">{user.username || user.email.split("@")[0]}</div>
                      ...
                      <td className="px-6 py-5 hidden md:table-cell whitespace-nowrap">
                        <span className="text-xs font-medium text-slate-400 font-mono bg-slate-100 px-2 py-1 rounded-md">{user._id.substring(user._id.length - 8)}</span>
                      </td>
                      <td className="px-6 py-5 whitespace-nowrap">
                        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${user.role === 'police' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : user.role === 'admin' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-slate-50 text-slate-600 border-slate-200'}`}>
                      ...
                      <td className="px-6 py-5 hidden lg:table-cell">
                        {user.policeVerification?.status === "pending" ? (
                           <div className="flex flex-col gap-1">
                             <span className="inline-flex items-center gap-1 text-[10px] font-black text-amber-500 uppercase tracking-widest"><ShieldAlert size={12} className="animate-pulse" /> Pending Review</span>
                             <span className="text-[10px] font-bold text-slate-400">Badge: {user.policeVerification?.badgeNumber || "NA"}</span>
                           </div>
                        ) : user.role === "police" ? (
                           <span className="inline-flex items-center gap-1 text-[10px] font-black text-emerald-500 uppercase tracking-widest"><CheckCircle2 size={12} /> Verified Officer</span>
                      ...
                      <td className="px-6 py-5 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-2">
                          {user.policeVerification?.status === "pending" && (
                            <>
                              <button onClick={() => verifyPoliceApplication(user._id, "approve")} className="px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-emerald-100 hover:border-emerald-500">Approve</button>
                              <button onClick={() => verifyPoliceApplication(user._id, "reject")} className="px-3 py-1.5 bg-rose-50 text-rose-600 hover:bg-rose-500 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-rose-100 hover:border-rose-500">Reject</button>
                            </>
                          )}
                          {user.role === "police" && (
                            <button onClick={() => removePolice(user._id)} className="px-3 py-1.5 bg-slate-50 text-slate-500 hover:bg-rose-50 hover:text-rose-600 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 hover:border-rose-200 inline-flex items-center gap-1.5"><XCircle size={14} /> Revoke</button>
                          )}
                          <button onClick={() => removeUserAction(user._id)} className="px-3 py-1.5 bg-slate-50 text-slate-400 hover:bg-rose-600 hover:text-white rounded-lg text-[10px] font-black uppercase tracking-widest transition-all border border-slate-200 hover:border-rose-600 inline-flex items-center gap-1"><Trash2 size={14} /> Remove</button>
                        </div>
                      </td>
                    </tr>
                  ))}
              ...
              {users.length === 0 && <div className="p-12 text-center text-slate-500 font-bold text-sm">No accounts found.</div>}
              
              {/* Server-Side Pagination Controls */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-t border-slate-200">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Page {currentPage} of {totalPages} ({totalItems} Users)</span>
                  <div className="flex items-center gap-2">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="px-3 py-1.5 text-xs font-black uppercase tracking-widest bg-white border border-slate-200 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Prev</button>
                    <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="px-3 py-1.5 text-xs font-black uppercase tracking-widest bg-white border border-slate-200 rounded-lg text-slate-500 hover:bg-slate-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Next</button>
                  </div>
                </div>
              )}
            </div>
